...
const Modal: FC<{
  ...
  const { data, isLoading } = useProfile(query?.id);
  ...
  const handleUpdate = async (payload: {
    ...
    const me = await fetch("/api/me/" + query?.id, {
      method: "PUT",
      headers: {
  ...
  const update = useMutation(handleUpdate, {
    onSuccess: () => {
      ...
      toast.success("Säljarens inställningar har sparats");
    },
    onError: (error: Error) => {
      console.log(error);
      toast.error("Säljarens inställningar kunde inte sparas");
    },
  });
  const deleteUser = useMutation(
    ...
    },
    {
      onSuccess: () => {
        ...
        router.push("/");
        queryClient.invalidateQueries(["profile"]);
        toast.success("Användaren har inaktiverats");
      },
      onError: (error: Error) => {
        console.log(error);
        toast.success("Användaren kunde inte inaktiveras");
      },
    }
  );
  const onSubmit: SubmitHandler<Inputs> = async ({
    ...
    if ((!imageName || !image || !imageUrl) && user?.avatar) {
      update.mutate({
        name,
        avatar: user?.avatar,
    ...
    } else if (imageName && image && imageUrl && user?.avatar) {
      ...
      if (error) {
        console.log(error);
        toast.error("Det gick inte att ladda upp användarens profilbild");
      }
      update.mutate({
        name,
        avatar: imageUrl,
      ...
      if (error) {
        console.log(error);
        toast.error("Det gick inte att uppdatera användarens profilbild");
      }
      update.mutate({
        name,
        avatar: imageUrl,
  ...
  const handleImage = async (event: FormEvent<HTMLInputElement>) => {
    ...
    let filename = user?.avatar
      ? user?.avatar.split("avatars/")[1].split("?")[0]
      : `${Math.random()}.${fileExt}`;
    setImage(file);
...
const Header: FC = () => {
  ...
  const query = router.query as { id: string | undefined };
  const profile = useProfile(query?.id);
  const queryClient = useQueryClient();
  ...
  const salesData = [
    {
      ...
      data: sales.data?.sales.filter(
        (sale: Sale) => sale.profileId === query?.id
      ).length
        ? sales.data?.sales
            .filter((sale: Sale) => sale.profileId === query?.id)
            .map((sale: Sale) => ({
              x: sale.date,
              y: sales.data?.sales
                ?.filter(
                  (item: Sale) =>
                    item.date === sale.date && item.profileId === query?.id
                )
  ...
  const offersData = [
    {
      ...
      data: offers.data?.offers.filter(
        (offer: Offer) => offer.profileId === query?.id
      ).length
        ? offers.data?.offers
            .filter((offer: Offer) => offer.profileId === query?.id)
            .map((offer: Offer) => ({
              x: offer.date,
              y: offers.data?.offers
                ?.filter(
                  (item: Offer) =>
                    item.date === offer.date && item.profileId === query?.id
                )
  ...
  const meetingsData = [
    {
      ...
      data: meetings.data?.meetings.filter(
        (meeting: Meeting) => meeting.profileId === query?.id
      ).length
        ? meetings.data?.meetings
            .filter((meeting: Meeting) => meeting.profileId === query?.id)
            .map((meeting: Meeting) => ({
              x: meeting.date,
              y: meetings.data?.meetings?.filter(
                (item: Meeting) =>
                  item.date === meeting.date && item.profileId === query?.id
              ).length,
            }))
            .reverse()
        : Array(7)
  ...
  const bookingsData = [
    {
      ...
      data: bookings.data?.bookings.filter(
        (booking: Booking) => booking.profileId === query?.id
      ).length
        ? bookings.data?.bookings
            .filter((booking: Booking) => booking.profileId === query?.id)
            .map((booking: Booking) => ({
              x: booking.date,
              y: bookings.data?.bookings?.filter(
                (item: Meeting) =>
                  item.date === booking.date && item.profileId === query?.id
              ).length,
            }))
            .reverse()
        : Array(7)
...
const Status: FC = () => {
  const router = useRouter();
  const query = router.query as { id: string | undefined };
  const { data } = useProfile(query?.id);
  return (
        ...
        <div className="overflow-hidden rounded-lg bg-gray-900 shadow xl:h-full">
          <div className="relative flex h-full flex-col items-center justify-center space-y-6 p-6">
            ...
            <p className="text-center font-semibold text-white sm:text-xl sm:font-bold">
              Säljarens poäng den här månaden ligger på <br />
